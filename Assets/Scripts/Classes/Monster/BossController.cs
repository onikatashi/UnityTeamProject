...
public class BossController : MonsterBase
{
    ...
    {
        // 현재 존재하는 Player 컴포넌트 중 레이어가 맞는 것 찾기
    ...
    IEnumerator Pattern_Laser()
    {
        ...
        while (t < laserDuration)
        {
            ...
            if (tick >= laserTick)
            {
                laser.TryHit(playerComp, md.attackDamage); // 틱뎀
                tick = 0f;
    ...
    IEnumerator CoPull(Transform target, float duration, float strength)
    {
        ...
        while (t < duration && target != null)
        {
            ...
            if (rb != null)
            {
                ...
                rb.linearVelocity = new Vector3(v.x, rb.linearVelocity.y, v.z); // 3D Rigidbody는 linearVelocity 사용
            }
            else
            {
    ...
    IEnumerator Pattern_ExecuteCharge()
    {
        ...
        {
            playerComp.TakeDamage(999999f); // 즉사 데미지로 처리
        }
    ...
    IEnumerator Pattern_JumpTop()
    {
        ...
        agent.isStopped = true;
        transform.position = topCenterPoint.position; // 점프(맵 상단 중앙으로)
        yield return new WaitForSeconds(0.35f);
    ...
    IEnumerator Pattern_BulletHell()
    {
        ...
        while (t < hellDuration * 0.34f)
        {
            ...
            if (fire >= hellFireInterval)
            {
                ...
                FireBulletAtAngle(spiralAngle + 90f);   // 90도
                FireBulletAtAngle(spiralAngle + 270f);  // 270도
            }
            yield return null;
        }
